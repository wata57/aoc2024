...
const diffIsRight = function (line) {
  ...
  while (r < line.length) {
    const diff = Math.abs(Number(line[r]) - Number(line[l]));
...
for (let row of data) {
  ...
  if (diffIsRight(line) && isSorted(line)) {
    res += 1;
    continue;
  }
  ...
  for (let j = 0; j < row.length; j++) {
    const newLine = (line.slice(0, j) + "," + line.slice(j + 1))
      .trim(",")
      .split(",")
      .filter((char) => char !== "");

    if (diffIsRight(newLine) && isSorted(newLine)) {
      isSafe = true;
      break;
    }
  }
